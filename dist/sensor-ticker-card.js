...
class SensorTickerCard extends HTMLElement {
  ...
  _resolveRows() {
    ...
    for (const s of cfg.sensors) {
      // Condition-based "show" filter
      if (s.show_if) {
        const { show_if } = s;
        const stateObj = hass.states[show_if.entity ?? s.entity];
        if (!stateObj) continue;
        if (!this._evalCondition(stateObj.state, show_if.op, show_if.value)) continue;
      }
  ...
  _refreshValues() {
    // Update row content without interrupting scroll animation
